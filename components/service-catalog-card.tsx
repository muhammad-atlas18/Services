...
export function ServiceCatalogCard({ service, position = "service_catalog" }: { service: CatalogService; position?: string }) {
  const badge = service.category === "solar" ? "Solar service" : service.category === "electrical" ? "Electrical service" : service.category === "ac" ? "AC service" : "Home appliance service";
  return <article className="catalogCard"><Link className="catalogImage" href={service.route} aria-label={`See details for ${service.title}`}><Image src={service.image} alt={service.imageAlt} fill sizes="(max-width: 700px) 100vw, (max-width: 1000px) 50vw, (max-width: 1280px) 33vw, 25vw" /></Link><div className="catalogBody"><div className="catalogTop"><span className="catalogIcon" aria-hidden="true"><CatalogIcon type={service.icon} /></span><span className="catalogBadge">{badge}</span></div><h3><Link href={service.route}>{service.title}</Link></h3><p>{service.summary}</p><div className="catalogActions"><Link className="button secondary" href={service.route}>See Details</Link><WhatsAppLink service={service.title} message={service.whatsappMessage} position={position}>Contact on WhatsApp</WhatsAppLink></div></div></article>;
}
