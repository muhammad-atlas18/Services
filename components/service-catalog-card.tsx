...
function CatalogIcon({ type }: { type: string }) {
  ...
  if (["refrigerator", "washing", "microwave", "dispenser", "oven", "dishwasher"].includes(type)) return <svg viewBox="0 0 24 24"><rect x="5" y="2.5" width="14" height="19" rx="2"/><path d="M5 9h14M9 6h.01M9 13h.01"/></svg>;
...
export function ServiceCatalogCard({ service, position = "service_catalog" }: { service: CatalogService; position?: string }) {
  const badge = service.category === "solar" ? "Solar Services" : service.category === "electrical" ? "Electrical Services" : service.category === "ac" ? "AC Services" : "Home Appliances";
  return <article className="catalogCard"><Link className="catalogImage" href={service.route} aria-label={`See service details for ${service.title}`}><Image src={service.image} alt={service.imageAlt} fill sizes="(max-width: 700px) 100vw, (max-width: 1000px) 50vw, (max-width: 1280px) 33vw, 25vw" /></Link><div className="catalogBody"><div className="catalogTop"><span className="catalogIcon" aria-hidden="true"><CatalogIcon type={service.icon} /></span><span className="catalogBadge">{badge}</span></div><h3><Link href={service.route}>{service.title}</Link></h3><p>{service.summary}</p><div className="catalogActions"><Link className="button secondary" href={service.route}>See Service Details</Link><WhatsAppLink service={service.title} serviceId={service.path} position={position}>Contact on WhatsApp for {service.title}</WhatsAppLink></div></div></article>;
}
