...
export function WhatsAppLink({ service, children = "Contact on WhatsApp", className = "button whatsappButton", position = "content" }: { service?: string; children?: React.ReactNode; className?: string; position?: string }) {
  const label = typeof children === "string" ? children : "Contact on WhatsApp";
  return <a aria-label={`Contact on WhatsApp${service ? ` about ${service}` : ""}`} className={`${className} whatsappAction`} target="_blank" rel="noreferrer" href={whatsappUrl(whatsappMessage(service))} onClick={() => track({ event: "whatsapp_click", page_path: window.location.pathname, service: service || "general", cta_position: position, device_type: window.matchMedia("(max-width: 767px)").matches ? "mobile" : "desktop" })}><WhatsAppIcon /><span>{label}</span></a>;
}
