...
export default function HomePage() {
  return <PageShell>
    <HomeHeroCarousel />
    <section id="services" className="serviceCatalogue"><div className="container"><div className="catalogueIntro"><span className="eyebrow">What we do</span><h2>Our Services</h2><p>Professional Solar, Electrical and AC services, organised by category for homes and businesses across Lahore.</p></div>{categoryOrder.map((category) => { const meta = getCategoryMeta(category); return <section className="catalogCategory" key={category} aria-labelledby={`${category}-services-heading`}><div className="catalogCategoryHeading"><span className="eyebrow">{category} category</span><h3 id={`${category}-services-heading`}>{meta.label}</h3><p>{meta.intro}</p></div><div className="catalogGrid">{servicesForCategory(category).map((service) => <ServiceCatalogCard service={service} position={`home_${category}_service`} key={service.path} />)}</div></section>; })}</div></section>
